...
namespace Roseworks
{
	public struct SRule
	{
		...
		*/
		...
		public SRule(Type comType, int comID = -1, int from = int.MaxValue, int to = int.MaxValue, Action<int, int> callback = null, Func<bool> callbackCnd = null, float duration = -1, int timerStart = -1, int timerID = -1, Func<bool> endCnd = null, FSM.EActive activeChecks = FSM.EActive.None, bool active = false)
		{
			ComType = comType;
			From = from;
			To = to;
