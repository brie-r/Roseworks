...
namespace Roseworks
{
	public static class Timer
	{
		...
		public static void Update(float time, float dt)
		{
			...
			while (i >= 0)
			{
				...
				if (GetTime(ref timer, time) > timer.StartTime + timer.Duration - dt / 2f)
				{
					...
					{
						Logger.Builder.Append(DFormatTime(time) + "\tTimer ended: " + Timers.IndicesToIds[i] + " entID: " + timer.ComID);
						Logger.Builder.Append((timer.Callback == null ? "\tNo callback invoked" : ("\tCallback invoked: " + timer.Callback.Method)));
						DPrintClear();
		...
		public static void Add(out int outTimerID, STimer timer, float time)
		{
			...
			Logger.Builder.Append("Timer added: id " + outTimerID + ", entID " + timer.ComID + ", time " + DFormatTime(timer.StartTime) + " - " + DFormatTime(timer.StartTime + timer.Duration));
			DPrintClear();
		}
		public static void Add(float time, out int outTimerID, int comID, float duration, float startTime = -1, System.Action<int> callback = null, bool autoCancel = true)
		{
			...
			Logger.Builder.Append("Timer added: id " + outTimerID + ", entID " + comID + ", " + DFormatTime(startTime) + " - " + DFormatTime(startTime+duration));
			DPrintClear();
		}
		public static void Add(int[] outTimerIDs, STimer[] timers, float time)
		{
			...
			for (int i = 0; i < timers.Length; i++)
			{
				...
				Logger.Builder.AppendLine("Timer added: id " + outTimerIDs[i] + ", entID " + timers[i].ComID + ", " + DFormatTime(timers[i].StartTime) + " - " + DFormatTime(timers[i].StartTime + timers[i].Duration));
				DPrintClear();
			}
		}
		public static void Add(float time, ref int[] outTimerIDs, int[] comID, float[] duration, float[] startTime, System.Action<int>[] callback = null, BitArray autoCancel = null)
		{
			...
			for (int i = 0; i < comID.Length; i++)
			{
				...
				Logger.Builder.AppendLine("Timer added: id " + outTimerIDs[i] + ", entID " + comID[i] + ", " + DFormatTime(startTime[i]) + " - " + DFormatTime(startTime[i] + duration[i]));
				DPrintClear();
			}
		}
		public static int[] AddTimers(List<int> comID, float time, List<float> duration, List<float> startTime, List<System.Action<int>> callback = null, BitArray autoCancel = null)
		{
			...
			for (int i = 0; i < startTime.Count; i++)
			{
				...
				Logger.Builder.AppendLine("Timer added: id " + timerIDs[i] + ", entID " + comID[i] + ", " + DFormatTime((float)startTime[i]) + " - " + DFormatTime((float)startTime[i] + duration[i]));
				DPrintClear();
			}
			return timerIDs;
		}
		...
		{
			Logger.Builder.AppendLine("\tCancel timers by comID: " + comID + ", ignore timerID " + ignoreID);
			for (int i = Timers.Count - 1; i >= 0; i--)
			{
				Logger.Builder.Append("Cancel " + Timers.IndicesToIds[i] + "? ");
				Logger.Builder.Append((Timers[i].ComID == comID) + " + " + (Timers.IndicesToIds[i] != ignoreID) + "\t");
		...
		private static bool DValidTimerIndex(int timerIndex)
		{
			...
			{
				Logger.WriteLine(s.GetFrame(1).GetMethod().Name + " Timer with index " + timerIndex + " not found.");
				DPrintClear();
				return false;
			}
			return true;
